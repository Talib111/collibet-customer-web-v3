...
const Marriage = () => {
					...
					<div className="overflow-hidden bg-black lg:mx-8 lg:flex lg:max-w-6xl lg:w-full lg:shadow-md lg:rounded-xl">
						<div className="lg:w-1/2">
							<div
								...
								style={{
									backgroundImage:
										"url('https://res.cloudinary.com/dcxnmagjx/image/upload/v1688842817/Rectangle_11_tbsjha.png')",
								}}
							></div>
						</div>
  ...
  <div className="space-x-50 md:space-x-0 md:space-y-50 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
    <Swiper slidesPerView={4}>
      {data.slice(0, data.length - 12).map((card, i) => (
        <SwiperSlide key={i}>
          <div className="p-4">
            <div className="flex-col-start r-card gap-2 bg-gray-200 rounded-lg max-w-max mx-auto transition-all duration-300 ease-in hover:scale-105 hover:cursor-pointer hover:bg-gradient-to-b from-transparent to-gray-400 hover:shadow-lg">
              ...
              <span className="flex text-md p-2 text-black">
                {card.detail}
              </span>
            </div>
          </div>
					...
					<div className="">
						<div className="grid max-w-lg grid-cols-4 items-center gap-x-1 gap-y-1 sm:max-w-xl sm:grid-cols-6 sm:gap-x-10 lg:mx-0 lg:max-w-none lg:grid-cols-4">
							<motion.img
								...
								src="https://res.cloudinary.com/dcxnmagjx/image/upload/v1688821535/1_k16ts8.png"
								alt="Transistor"
								width={250}
								height={250}
							/>
